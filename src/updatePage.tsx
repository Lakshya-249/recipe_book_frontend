...
const UpdatePage = () => {
  ...
  useEffect(() => {
    const change = () => {
      for (let detail in details) {
        if (typeof details[detail] === "number") continue;
        // console.log(details);
  ...
  const handleClick = async () => {
    ...
    if (field) {
      // console.log(details);
      setfield(false);
      const response = await fetch(api + "crecipe/" + id + "/", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="flex flex-col items-stretch w-full px-10 py-6 font-sans">
      <div className="w-[50%] self-center text-left text-pretty max-sm:w-full">
        <p className="text-[2.5rem] font-black mb-3">Welcome Back!</p>
        <p className="text-md font-semibold text-gray-500 mb-8">
          Let Update you recipe
        </p>
        <p className="text-xl font-bold mb-6">Add new pic of your food</p>
        ...
        <textarea
          cols={30}
          defaultValue={details.ingrediants}
          name="ingrediants"
          rows={10}
          onChange={(e) => setData(e)}
